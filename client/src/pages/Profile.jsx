...
const Profile = () => {
  ...
  const {authUser, updateProfile} = useContext(null);
      ...
      <div className='w-5/6 max-w-2xl backdrop-blur-2xl text-gray-300 border-2 border-gray-600 flex items-center justify-between max-sm:flex-col-reverse rounded-lg'>
        <form onSubmit={handelSubmit} className="flex flex-col gap-5 p-10 flex-1">
          ...
          <label htmlFor="avatar" className='flex items-center gap-3 cursor-pointer'>
            ...
            <img src={selectedImg ? URL.createObjectURL(selectedImg) : assets.avatar_icon} alt="" className={`w-12 h-12 ${selectedImg && 'rounded-full'}`}/> {/* ✅ FIX 1: moved image inside label, FIX 2: added correct src */}
            upload profile image {/* ✅ FIX: moved inside label so clicking text also opens file picker */}
          </label>
          ...
          <textarea onChange={(e)=> setBio(e.target.value)} value={bio} placeholder='Write profile bio' required className="p-2 border border-gray-500 rounded-mg focus:outline-none focus:ring-2 focus:ring-violet-500" rows={4}></textarea>
